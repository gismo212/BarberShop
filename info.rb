...
Синтаксис before (в sinatra) - исполняет код перед запросом - будет доступно во всех представлениях(view)
before do
  # some code
end
	<tbody>
...
метод configure выбирается для конфигурации приложений (если приложение обновилось )
...
способ создания второй таблицы через configure db.execute 'CREATE TABLE IF NOT EXISTS "Barbers"
	(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT,
	 "name" TEXT)'
все!!
